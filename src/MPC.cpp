...
std::vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  retVector.push_back(solution.x[delta_start]);
  retVector.push_back(solution.x[a_start]);

  for(size_t idx=0; idx< N-1; idx++){
    retVector.push_back(solution.x[x_start + idx + 1]);
    retVector.push_back(solution.x[y_start + idx + 1]);
  }
